...
注意CFRunLoopModeRef虽然暴露，但是是通过简介暴露的方式暴露出去的，通过结构体
__CFRunLoopMode 暴露的，从下面可以看到它的内容：

结合官方文档，可以知道RunLoop和LoopMode的关系是，一个RunLoop 包含若干个Model
每个 Mode 又包含若干个 Source/Timer/Observer。每次调用 RunLoop 的主函数时，只能指定其中一个 Mode，这个Mode被称作 CurrentMode。如果需要切换 Mode，只能退出 Loop，再重新指定一个 Mode 进入。
...
/*
    mode item包含Source/Timer/Observer，让其互不影响
...
//苹果不允许直接创建RunLoop、它只提供了两个自动获取的含税
//CFRunLoopGetMain() 和CFRunLoopGetCurrent(),以下是两个函数的实现
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    if (pthread_equal(t, pthread_self())) {
        ...
        if (0 == _CFGetTSD(__CFTSDKeyRunLoopCntr)) {
            //注册一个回调，当线程销毁时，顺便也销毁器对应的RunLoop
...
从上面的代码中可以看出，线程和Runloop之间是一一对应的关系的，
其关系是保存在一个全局的 Dictionary 里。线程刚创建时并没有 RunLoop，
如果你不主动获取，那它一直都不会有。RunLoop 的创建是发生在第一次获取时，
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
	...
	__CFPortSet waitSet = rlm->_portSet;
        ...
        2.通知Observers：RunLoop 即将触发Timer回调
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        3.通知Observers：RunLoop：Runloop 即将触发Source0 回调，非port回调
...
RunLoop触发Source0（非port）回调
        ...
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
            ...
            如果有Source1（基于port）处于ready状态，直接跳转去处理这个消息
...
通知Observers：RunLoop的线程即将进入休眠sleep
	...
	__CFRunLoopUnlock(rl);
        ...
        do {
            ...
            msg = (mach_msg_header_t *)msg_buffer;
            调用mach_msg 等待接受mach_port 的消息，线程将进入休眠
            直到下面某一个事件唤醒
            - 一个基于port的Source的事件
            - 一个Timer到时间了
            - RunLoop自身的超时时间到了
...
收到消息，处理消息
        ...
        if (MACH_PORT_NULL == livePort) {
            ...
            如果一个Source1 基于port发出事件了，处理这个事件
    ...
    如果没有超时，mode里没空，loop也没被停止，那继续loop
    } while (0 == retVal);
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
	...
	if (currentMode->_observerMask & kCFRunLoopEntry )
     __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry); 通知Observers，RunLoop即将进入loop
    内部函数，进入loop
